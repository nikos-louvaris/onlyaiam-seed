...
> **Πειθαρχία → μηχανισμός (ώριμο):** το «Ποτέ MCP-by-default» δεν είναι πια μόνο πειθαρχία — είναι rail. `reflex/prefer-official-source.sh "<κατηγορία>"` καλείται πριν συνδεθεί οτιδήποτε: αν ζητείται MCP ενώ υπάρχει επίσημος CLI/API δρόμος στον χάρτη, επιστρέφει exit 1 (στάση), δείχνει την επίσημη πηγή, και απαιτεί ρητό `--force` με τεκμηρίωση. Ο σπόρος έκλεισε τον δικό του scar όπως λέει το SOUL: «αν χρειάζεται enforcement → config/code, όχι πειθαρχία.»
...
- Πάει να μπει **MCP** → `bash reflex/prefer-official-source.sh "<κατηγορία>"` πρώτα. exit 1 = υπάρχει επίσημος δρόμος, χρησιμοποίησέ τον· `--force` μόνο με τεκμηριωμένη εξαίρεση.
