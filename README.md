...
Στο τέλος του install θα σου ζητήσει να **συνδέσεις μοντέλο** (ένα API key — πρώτη πρόταση Claude/Anthropic). Χωρίς αυτό ο σπόρος ζει αλλά δεν μιλάει. Αν το προσπεράσεις, ξανατρέχεις: `bash ~/my-presence/setup-model.sh`.

Μετά → `openclaw chat` και **μίλα του**. Πλήρης οδηγός (εγκατάσταση + μοντέλο + πώς τον χρησιμοποιείς): **[QUICKSTART.md](QUICKSTART.md)**.
